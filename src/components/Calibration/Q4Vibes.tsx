...
export default function Q4Vibes({ profile, onNext, onSkip }: Props) {
  const total = Q4_VIBE_OPTIONS.length;
  const [focusIdx, setFocusIdx] = useState(0);
  const [selected, setSelected] = useState<string[]>([]);

  const buildIdx = total;
  const skipIdx = total + (selected.length > 0 ? 1 : 0);
  const totalSlots = total + (selected.length > 0 ? 1 : 0) + 1;
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (focusIdx < total) {
        ...
        if (e.key === 'ArrowDown') { e.preventDefault(); setFocusIdx(i => Math.min(total - 1, i + COLS) < total ? i + COLS : total); }
        if (e.key === 'ArrowUp') { e.preventDefault(); setFocusIdx(i => Math.max(0, i - COLS)); }
        if (e.key === 'Enter' || e.key === ' ') {
          e.preventDefault();
          const id = Q4_VIBE_OPTIONS[focusIdx].id;
          setSelected(prev => {
  ...
  const handleBuild = () => {
    let p: PreferenceProfile = {
      ...
      selectedQ4Vibes: selected,
    };
    for (const id of selected) {
      const opt = Q4_VIBE_OPTIONS.find(o => o.id === id)!;
    ...
    >
      ...
      <div style={{ textAlign: 'center', marginBottom: 36, zIndex: 1 }}>
        ...
        <h2 style={{ fontSize: 46, fontWeight: 800, color: '#F5F3F7', margin: '0 0 10px', letterSpacing: -0.5 }}>
          Pick the worlds that feel like you
        </h2>
        <div style={{ fontSize: 18, color: 'rgba(167,134,229,0.7)' }}>
          Up to {MAX_SELECT} · {selected.length}/{MAX_SELECT} selected
        </div>
      </div>
          ...
          return (
            ...
            >
              ...
              <div style={{ position: 'absolute', bottom: 14, left: 14, right: 14 }}>
                <div style={{ fontSize: 16, fontWeight: 700, color: '#F5F3F7' }}>{opt.label}</div>
              </div>
            </div>
          );
        })}
